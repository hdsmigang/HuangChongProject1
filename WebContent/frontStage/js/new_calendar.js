    ...
    var execCalendar = function(input,option){
        ...
        var calendar = new calendarV2('#index_list_calendar-box', {
            checkDayChange:function(){
				...
				//计算共多少晚
				var startdate = new Date($('#checkinday_value').val()); 
				var enddate = new Date($('#checkoutday_value').val());
                var totalDays = parseInt((enddate.getTime() - startdate.getTime()) / (1000 * 60 * 60 * 24));
